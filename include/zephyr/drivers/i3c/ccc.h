...
#define I3C_CCC_ENEC_EVT_ENINT		BIT(0)
#define I3C_CCC_ENEC_EVT_ENCR		BIT(1)
#define I3C_CCC_ENEC_EVT_ENHJ		BIT(3)

#define I3C_CCC_DISEC_EVT_DISINT	BIT(0)
...
/**
 * @brief Payload for GETACCCR CCC (Get Accept Controller Role).
 *
 * Note that the target address is encoded within
 * struct i3c_ccc_target_payload instead of being encoded in
 * this payload.
 */
struct i3c_ccc_getacccr {
	/**
	 * Address of the new Active Controller.
	 *
	 * @note The address is left-shift by 1, and bit[0]
	 * is the calculated odd parity bit.
	 */
	uint8_t addr;
} __packed;
 ...
 *
 * This tests if the CCC payload is for broadcast. If this is a broadcast
 * payload, the number of targets is zero, meaning that
 *
 * @retval true if payload target is broadcast
 * @retval false if payload target is direct
 */
static inline bool i3c_ccc_is_payload_broadcast(struct i3c_ccc_payload *payload)
{
 ...
 *
 * @param[in] controller Pointer to the controller device driver
 *            instance.
 *
 * @return @see i3c_do_ccc
 */
