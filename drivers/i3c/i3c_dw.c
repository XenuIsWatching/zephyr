...
struct dw_i3c_config {
	...
	uint32_t od_thigh_max_ns; // Maximum OD high clk pulse length
	uint32_t od_tlow_min_ns;  // Minumum OD low clk pulse length
...
static void dw_i3c_end_xfer(const struct device *dev)
{
	...
	for (i = 0; i < nresp; i++) {
		...
		if (tid == 0xf) {
			/* TODO: handle vendor extenstion ccc or hdr header in target mode */
			continue;
		}
...
static void start_xfer(const struct device *dev)
{
	...
	for (i = 0; i < xfer->ncmds; i++) {
		cmd = &xfer->cmds[i];
		if (cmd->buf != NULL) { // Not all the commands use write_tx_fifo function
...
static void dw_i3c_handle_tir(const struct device *dev, uint32_t ibi_status)
{
	...
	int pos = get_i3c_addr_pos(dev, addr);
	if (pos < 0) {
		LOG_ERR("%s Invalid Slave address", __func__);
		return;
	}
...
static void ibis_handle(const struct device *dev)
{
	...
	for (i = 0; i < nibis; i++) {
		...
		if (IBI_TYPE_SIRQ(ibi_stat)) {
			...
			LOG_DBG("Secondary Master Request Not implemented");
		}
	}
}
				...
				uint8_t addr)
{
	...
	LOG_DBG("Attaching %s", desc->dev->name);
...
static int dw_i3c_detach_device(const struct device *dev, struct i3c_device_desc *desc)
{
	...
	LOG_DBG("Detaching %s", desc->dev->name);
...
// return 0 if input is odd, return 1 if input is even.
...
static int dw_i3c_do_ccc(const struct device *dev, struct i3c_ccc_payload *payload)
{
	...
	if (xfer->cmds[0].error == RESPONSE_ERROR_IBA_NACK) {
		cmd->error = I3C_ERROR_TE2; // TODO: is this needed??
...
#define I3C_DW_IRQ_HANDLER(n)                                                                      \
	static void i3c_dw_irq_config_##n()                                                        \
	{                                                                                          \
