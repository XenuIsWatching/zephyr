			       ...
			       bool asynchronous, spi_callback_t cb, void *userdata)
{
	...
	uint32_t dfs = SPI_WORD_SIZE_GET(data->ctx.config->operation) / 8;
 ...
 */
static struct spi_driver_api spi_cdns_api = {
...
#define SPI_CDNS_CLK_DEFINE(n) CLOCK_MANAGEMENT_DT_INST_DEFINE_OUTPUT(n)
#define SPI_CDNS_CLK_INIT(n)                                                                       \
	.clock_output = CLOCK_MANAGEMENT_DT_INST_GET_OUTPUT(n),                                    \
	.clock_default_state = CLOCK_MANAGEMENT_DT_INST_GET_STATE(n, default, default),            \
	.clock_sleep_state = CLOCK_MANAGEMENT_DT_INST_GET_STATE(n, default, sleep),
#endif
